...
- **R5-01, HIGH, outcome-determining. R4-01 reopens.** `tq-gate-baseline.js` requires a matrix row to name its own lines but supplies nothing that derives or validates them, so the class of citation that decides LINT-14 is the class nothing checks. Four of four scoped matrix regressions were false positives and two more could not be scoped at all, against four of four LINT regressions correct. A matrix row needs a derived, narrow, validated line source, or an unscoped or hand-sourced matrix row must not decide LINT-14 by itself.
...
- **R5-03, LOW.** The justification at `:758-759` says "on text the revision changed" regardless of how many regressions were unscoped, and that sentence is pinned into the evidence record. One clause carrying the unscoped count fixes it.
