...
**What was changed afterwards.** Nothing during the run; the plugin stayed frozen. Afterwards, two of the six defects were worked: R5-03 is closed and R5-01 is half closed, both in `tq-gate-baseline.js`, guarded by 18 new assertions in `tests/gate-baseline-test.js` (layer 9, now 252). R5-02, R5-04, R5-05 and R5-06 are untouched. **The headline number does not move for either.** R5-03 was LOW and changes what a record says rather than what the gate decides. R5-01's fix is verified as a mechanism and reaches no matrix row until one emits a record, so the deployed D10 classifier component stays at 60% — the four false positives would recur unchanged if s1 were re-run today. Raising it needs a sixth run against auditors that emit matrix anchors, which is the same evidence R5-01's remainder needs.
