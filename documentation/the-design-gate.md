...
**What the diff actually marks.** Three things: the lines the revision changed, the seams of any run whose position shifted, and every line that a relocated block crossed. The third is what makes a reordering visible — move a commit phase above a validation phase, and a concern about that ordering is a regression even though the validation line it cites was not itself edited. A moved block is identified by content, so an ordinary revision that inserts and edits in two places does not trigger it.

**The limit worth knowing before you cite a line.** The diff cannot show that a requirement was *unaffected* when only the prose around it was rewritten. Blocks that merely resemble each other do not pair as a move, and identical text appearing several times leaves its provenance ambiguous. So auditor variance means "no line this element cites lies in the diff" — not "the revision did not disturb this obligation". Cite the lines the obligation really rests on, including the heading that governs them.
