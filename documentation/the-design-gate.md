...
**What the diff actually marks.** Three things: the lines the revision changed, the seams of any run whose position shifted, and every line that a relocated block crossed. The third is what makes a reordering visible — move a commit phase above a validation phase, and a concern about that ordering is a regression even though the validation line it cites was not itself edited.

A block is recognised as relocated from lines that are *distinctive*: text appearing exactly once in each version. Boilerplate is therefore never evidence of a move, which matters more than it sounds — without that rule, relocating a single `---` in a document holding forty of them marks 334 of 361 lines, and deleting one blank line marks 145 of 150. Pairing happens line by line rather than whole-block, so adding a revision note beside the block that moved does not hide the move.

**The limit worth knowing before you cite a line.** The diff cannot show that a requirement was *unaffected* when only the prose around it was rewritten, and repeated text has no provenance to read. So auditor variance means "no line this element cites lies in the diff" — not "the revision did not disturb this obligation". Cite the lines the obligation really rests on, including the heading that governs them.
